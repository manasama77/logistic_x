							...
							<div class="row">
								<div class="col-sm-12 col-md-3">
									<div class="form-group">
										...
										<input type="text" class="form-control" id="code" name="code" placeholder="Kode Barang" value="<?= $arr->row()->code; ?>" required />
										<input type="hidden" id="code_old" name="code_old" value="<?= $arr->row()->code; ?>" required />
									</div>
									<div class="form-group">
										<label for="name">Nama Barang <span class="text-danger">*</span></label>
										<input type="text" class="form-control" id="name" name="name" placeholder="Nama Barang" value="<?= $arr->row()->name; ?>" required />
									</div>
									<div class="form-group">
										<label for="merk">Merk Barang <span class="text-danger">*</span></label>
										<input type="text" class="form-control" id="merk" name="merk" placeholder="Merk Barang" value="<?= $arr->row()->merk; ?>" required />
									</div>
								</div>
								<div class="col-sm-12 col-md-3">
									<div class="form-group">
										...
										<select class="form-control" id="category_id" name="category_id" required>
											...
											<?php foreach ($category->result() as $key) { ?>
												<option <?= ($arr->row()->category_id == $key->id) ? "selected" : null; ?> value="<?= $key->id; ?>"><?= $key->name; ?></option>
											<?php } ?>
										</select>
									</div>
									<div class="form-group">
										...
										<select class="form-control" id="unit_id" name="unit_id" required>
											...
											<?php foreach ($unit->result() as $key) { ?>
												<option <?= ($arr->row()->unit_id == $key->id) ? "selected" : null; ?> value="<?= $key->id; ?>"><?= $key->name; ?></option>
											<?php } ?>
										</select>
									</div>
									<div class="form-group">
										...
										<select class="form-control" id="supplier_id" name="supplier_id" required>
											...
											<?php foreach ($supplier->result() as $key) { ?>
												<option <?= ($arr->row()->supplier_id == $key->id) ? "selected" : null; ?> value="<?= $key->id; ?>"><?= $key->name; ?></option>
											<?php } ?>
										</select>
									</div>
									<div class="form-group">
										...
										<select class="form-control" id="location_id" name="location_id" required>
											...
											<?php foreach ($location->result() as $key) { ?>
												<option <?= ($arr->row()->location_id == $key->id) ? "selected" : null; ?> value="<?= $key->id; ?>"><?= $key->name; ?> - <?= $key->shelf_number; ?></option>
											<?php } ?>
										</select>
									</div>
								</div>
								<div class="col-sm-12 col-md-3">
									<div class="form-group">
										...
										<input type="hidden" id="image_path_old" name="image_path_old" value="<?= $arr->row()->image_path; ?>" required />
									</div>
								</div>
								<div class="col-sm-12 col-md-3">
									<?php
									$image_path = ($arr->row()->image_path) ? base_url('public/img/barang/' . $arr->row()->image_path) : base_url('public/img/barang/default.jpg');
									$disabled = ($arr->row()->image_path) ? null : "disabled";
									?>
									<img src="<?= $image_path; ?>" class="img-thumbnail elevation-2" id="preview_image" alt="Preview Image" />
